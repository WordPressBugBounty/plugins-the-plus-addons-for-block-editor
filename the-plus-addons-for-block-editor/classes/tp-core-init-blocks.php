...
class Tpgb_Core_Init_Blocks
{
	...
	public function editor_assets()
	{
		...
		$wp_localize_tpgb = array(
			...
			'isSproutDev' => defined('TPGB_SPROUT_DEV') && TPGB_SPROUT_DEV ? '1' : '0',
		);
	...
	public function plus_register_api_hook()
	{
		...
		//Sprout API
		if (defined('TPGB_SPROUT_DEV') && TPGB_SPROUT_DEV) {
			register_rest_route('tpgb/v1', '/nxt_sprout_api', array(
				'methods' => 'POST',
				'callback' => array($this, 'nxt_sprout_api'),
				'permission_callback' => function () {
				return current_user_can('edit_posts');
			}
			));
	 ...
	 * @since 10.0.0
	 */
	public function nxt_sprout_api( WP_REST_Request $request ) {

		$nonce = $request->get_param('nonce');

		if ( ! wp_verify_nonce( $nonce, 'nexter_admin_nonce' ) ) {
			return new WP_Error(
				'invalid_nonce',
		...
		$gutenbergCode = $request->get_param('gutenbergCode');
		$combination = $request->get_param('combination');
		if ( empty( $token_id ) || empty( $template_id ) || empty( $gutenbergCode ) ) {
			return new WP_Error(
				'missing_fields',
		...
		if ( ! is_array( $gutenbergCode ) ) {
			return new WP_Error(
				'invalid_payload',
				'gutenbergCode must be an array',
				array( 'status' => 400 )
			);
		}

		$post_data = isset( $gutenbergCode['post'] ) && is_array( $gutenbergCode['post'] )
			? $gutenbergCode['post']
			: array();

		$content = isset( $gutenbergCode['content'] ) && is_string( $gutenbergCode['content'] )
			? wp_unslash( $gutenbergCode['content'] )
			: '';

		$metadata = isset( $gutenbergCode['metadata'] ) && is_array( $gutenbergCode['metadata'] )
			? $gutenbergCode['metadata']
			: array();

		$content = $this->nxt_normalize_gutenberg_content( $content );

		$parsed_blocks = parse_blocks( $content );
		$readable_blocks = $this->nxt_make_blocks_readable( $parsed_blocks );

		$formatted_gutenberg = array(
			'post'            => $post_data,
			'content'  => $readable_blocks,
			'metadata'        => $metadata,
		);

		$api_url = 'https://label-studio.sproutui.com/api/trpc/template.postCodetoTemplate';

		$body_data = array(
			'id'            => $template_id,
			'gutenbergCode' => wp_json_encode(
				$formatted_gutenberg,
		...
		if( !empty( $combination ) ){
			$body_data['combination'] = $combination;
		}
		...
		if ( 200 !== (int) $http_code ) {
			return new WP_Error(
				'external_api_error',
				'External API returned ' . $http_code . ' - ' . $body,
	...
	/**
	 * Normalize gutenberg content
	 * @since 4.7.2
	 */
	private function nxt_normalize_gutenberg_content( $content ) {

		$content = wp_unslash( $content );

		$content = str_replace(
			array( '\u002d\u002d', 'u002du002d', '\u002d', 'u002d' ),
			array( '--', '--', '-', '-' ),
			$content
		);
	...
	/**
	 * Make blocks readable
	 * @since 4.7.2
	 */
	private function nxt_make_blocks_readable( $blocks ) {
		...
		foreach ( $blocks as $block ) {
			...
			if ( ! empty( $block['innerHTML'] ) ) {
				$item['innerHTML'] = trim( $block['innerHTML'] );
				$item['text']      = trim( wp_strip_all_tags( $block['innerHTML'] ) );
			}

			if ( ! empty( $block['innerBlocks'] ) && is_array( $block['innerBlocks'] ) ) {
				$item['innerBlocks'] = $this->nxt_make_blocks_readable( $block['innerBlocks'] );
			}
