...
wp_register_ability(
	...
	array(
		...
		'description'         => __( 'Adds a NEXTER BLOCKS Heading block (tpgb/tp-heading). Use this — NOT the WordPress core heading block — whenever the user asks to add a heading on a Nexter Blocks site. Typography is set via top-level params (enableTypography, fontFamily, fontType, fontWeight, textDecoration, lineHeight, letterSpacing, typoSize) — pass them directly here, do not route them through settings.tTypo. Also supports colours, stroke, shadow, blend mode, animations, transforms, spacing, background, border, and advanced position/visibility controls. ⚠️ NEXTER BUILD RULE — Nexter blocks render from their SAVED block HTML, not from attributes alone. Add and nest ONLY via nexter-blocks/add-tpgb-* abilities: to place this heading inside a container, pass that container\'s block_id as parent_block_id. NEVER build, rebuild, or edit a page that contains Nexter blocks with generic page-builder tools (e.g. assemble-page, patch-tree, html-to-builder, write-builder-content, place-widget) — they store attribute-only blocks with no inner HTML, so the heading renders blank. When finished, call nexter-blocks/verify-page to confirm no Nexter block is blank.', 'the-plus-addons-for-block-editor' ),
