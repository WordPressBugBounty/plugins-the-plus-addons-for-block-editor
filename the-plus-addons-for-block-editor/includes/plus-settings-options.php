...
function tpgb_activate_content(){
	echo '<div class="tp-pro-note-title"><p style="margin-bottom:40px;">'.esc_html__('Activate Licence to get access of our Pro Blocks, Features and Design Templates.','tpgb').'</p></div>
		<div style="text-align:center;">
			<img style="width: auto;" src="'.esc_url(TPGB_URL .'assets/images/activate.png').'" alt="'.esc_attr__('Activate','tpgb').'" class="panel-plus-activate" />
		</div>
		<div style="display: flex;flex-direction: column;align-items: center;">
            <div class="tpgb-pro-note-step">
                <span>STEP I : </span>
                <span>
                    <a href="'.esc_url('https://theplusblocks.com/pricing/').'" target="_blank" rel="noopener noreferrer">
            ...
            <div class="tpgb-pro-note-step">
                <span>STEP II : </span>
                <span>
                    <a href="'.esc_url('https://store.posimyth.com/dashboard/').'" target="_blank" rel="noopener noreferrer">
            ...
            <div class="tpgb-pro-note-step">
                <span>STEP III : </span>
                <span>
                    <a href="'.esc_url('https://theplusblocks.com/docs/how-to-activate-the-plus-addons-for-block-editor/').'" target="_blank" rel="noopener noreferrer">
					...
					echo '<div class="tpgb-onboarding-details">';
						echo '<div class="tpgb-theme-details mt-15">';
							echo '<div class="tpgb-theme-left tpgb-wd-45 mt-20">';
								...
								echo '<div class="tpgb-nxt-load"><img decoding="async" src="'.site_url().'/wp-includes/images/spinner.gif" alt="spinner.gif"></div>';
								echo '</div>';
							echo '</div>';
							echo '<div class="tpgb-theme-right tpgb-wd-55">';
...
class Tpgb_Gutenberg_Settings_Options {
    ...
    public function __construct() {
		if( is_admin() ){
			...
			add_action( 'admin_post_tpgb_connection_data_save', array( $this,'tpgb_connection_data_save_action') );
			add_action( 'admin_post_tpgb_custom_css_js_save', array( $this,'tpgb_custom_css_js_save_action') );
			add_action( 'admin_post_tpgb_white_label_save', array( $this,'tpgb_white_label_save_action') );
			...
			Tpgb_Library()->remove_backend_dir_files();
				if ( FALSE === get_option($action_page) ){
					...
					if(isset($_POST['tp_extra_option']) && !empty($_POST['tp_extra_option'])){
						if(is_array($_POST['tp_extra_option'])){
							$update_extra_val = array('tp_extra_option' => map_deep( wp_unslash( $_POST['tp_extra_option'] ), 'sanitize_text_field' ));
						}else{
							$update_extra_val = array('tp_extra_option' => sanitize_text_field($_POST['tp_extra_option']) );
						}
					}
					
					$block_value = array_merge( $update_value , $update_extra_val );
					update_option( $action_page, $block_value );
					wp_redirect( esc_url( admin_url('admin.php?page='.$action_page) ) );
				}
			}
	...
	public function tpgb_connection_data_save_action(){
		...
		if(isset($_POST["submit-key"]) && !empty($_POST["submit-key"]) && $_POST["submit-key"]=='Save'){
			...
			if ( ! isset( $_POST['nonce_tpgb_connection_data'] ) || ! wp_verify_nonce( sanitize_key($_POST['nonce_tpgb_connection_data']), 'nonce_tpgb_connection_data_action' ) ) {
				wp_redirect( esc_url(admin_url('admin.php?page='.$action_page)) );
			} else {
				$getArr = $_POST;
	...
	public function tpgb_custom_css_js_save_action(){
		...
		if(isset($_POST["submit-key"]) && !empty($_POST["submit-key"]) && $_POST["submit-key"]=='Save'){
			if ( ! isset( $_POST['nonce_tpgb_custom_css_js'] ) || ! wp_verify_nonce( sanitize_key($_POST['nonce_tpgb_custom_css_js']), 'nonce_tpgb_custom_css_js_action' ) ) {
				wp_redirect( esc_url(admin_url('admin.php?page='.$action_page)) );
			} else {
				$getArr = $_POST;
	...
	public function tpgb_white_label_save_action(){
		...
		if(isset($_POST["submit-key"]) && !empty($_POST["submit-key"]) && $_POST["submit-key"]=='Save'){
			...
			if ( ! isset( $_POST['nonce_tpgb_white_label'] ) || ! wp_verify_nonce( sanitize_key($_POST['nonce_tpgb_white_label']), 'nonce_tpgb_white_label_action' ) ) {
				wp_redirect( esc_url(admin_url('admin.php?page='.$action_page)) );
			} else {
				$getArr = $_POST;
	...
	public function block_listout(){
		$this->block_lists = [
			...
			'tp-button-core' => [
				...
				'videoUrl' => '',
				'tag' => 'free',
				'icon' => '<svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M30 12H6C4.89543 12 4 12.8954 4 14V22C4 23.1046 4.89543 24 6 24H30C31.1046 24 32 23.1046 32 22V14C32 12.8954 31.1046 12 30 12ZM6 10C3.79086 10 2 11.7909 2 14V22C2 24.2091 3.79086 26 6 26H30C32.2091 26 34 24.2091 34 22V14C34 11.7909 32.2091 10 30 10H6Z" fill="#5900E7"/><rect x="9" y="17" width="18" height="2" rx="1" fill="#1DD8A7"/></svg>',
			...
			'tp-dynamic-device' => [
				...
				'videoUrl' => '',
				'tag' => 'pro',
				'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 36 36"><rect width="2" height="6" x="15" y="25" fill="#1DD8A7" rx="1"/><rect width="2" height="10" x="21" y="29" fill="#1DD8A7" rx="1" transform="rotate(90 21 29)"/><path fill="#5900E7" fill-rule="evenodd" d="M2 8.143C2 6.407 3.343 5 5 5h21c1.657 0 3 1.407 3 3.143V13h-2V8.143c0-.579-.448-1.048-1-1.048H5c-.552 0-1 .47-1 1.048v15.714c0 .579.448 1.048 1 1.048h18V27H5c-1.657 0-3-1.407-3-3.143V8.143Z" clip-rule="evenodd"/><rect width="10" height="18" x="23" y="12" stroke="#1DD8A7" stroke-width="2" rx="1"/></svg>',
			...
			'tp-heading' => [
				...
				'videoUrl' => '',
				'tag' => 'free',
				'icon' => '<svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M10 4L10 15H26V4H23C22.4477 4 22 3.55228 22 3C22 2.44772 22.4477 2 23 2H26H28H31C31.5523 2 32 2.44772 32 3C32 3.55228 31.5523 4 31 4H28V15V17L28 28H31C31.5523 28 32 28.4477 32 29C32 29.5523 31.5523 30 31 30H28H26H23C22.4477 30 22 29.5523 22 29C22 28.4477 22.4477 28 23 28H26V17H10L10 28H13C13.5523 28 14 28.4477 14 29C14 29.5523 13.5523 30 13 30H10H8H5C4.44772 30 4 29.5523 4 29C4 28.4477 4.44772 28 5 28H8V17V15L8 4H5C4.44772 4 4 3.55228 4 3C4 2.44772 4.44772 2 5 2H8H10H13C13.5523 2 14 2.44772 14 3C14 3.55228 13.5523 4 13 4H10Z" fill="#5900E7"/><rect x="4" y="32" width="28" height="2" rx="1" fill="#1DD8A7"/></svg>',
			...
			'tp-icon-box' => [
				...
				'videoUrl' => '',
				'tag' => 'free',
				'icon' => '<svg viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M31 5H5L5 31H31V5ZM5 3C3.89543 3 3 3.89543 3 5V31C3 32.1046 3.89543 33 5 33H31C32.1046 33 33 32.1046 33 31V5C33 3.89543 32.1046 3 31 3H5Z" fill="#5900E7"/><path fill-rule="evenodd" clip-rule="evenodd" d="M20.9913 14.8323L18 7L15.0087 14.8323L7 15.4033L13.16 20.8148L11.2016 29L18 24.5123L24.7984 29L22.84 20.8148L29 15.4033L20.9913 14.8323ZM24.0861 17.058L19.5777 16.7366L18 12.6057L16.4223 16.7366L11.9139 17.058L15.3856 20.1079L14.3247 24.5419L18 22.1158L21.6753 24.5419L20.6144 20.1079L24.0861 17.058Z" fill="#1DD8A7"/></svg>',
			...
			'tp-image' => [
				...
				<path fill-rule="evenodd" clip-rule="evenodd" d="M33 7C33.5523 7 34 7.44772 34 8V29C34 30.6569 32.6569 32 31 32H6C5.44772 32 5 31.5523 5 31C5 30.4477 5.44772 30 6 30H31C31.5523 30 32 29.5523 32 29V8C32 7.44772 32.4477 7 33 7Z" fill="#1DD8A7"/><path fill-rule="evenodd" clip-rule="evenodd" d="M28 6H4V24.5347L9.13005 16.284C9.71066 15.3502 11.0654 15.3378 11.663 16.2608L14.4402 20.5502L20.1054 11.2701C20.7273 10.2513 22.2322 10.3318 22.742 11.411L28 22.5418V6ZM27.4217 26H5.44401L10.4124 18.0092L13.1935 22.3046C13.7941 23.2321 15.1572 23.214 15.733 22.2709L21.3295 13.1033L27.4217 26ZM4 4C2.89543 4 2 4.89543 2 6V26C2 27.1046 2.89543 28 4 28H28C29.1046 28 30 27.1046 30 26V6C30 4.89543 29.1046 4 28 4H4ZM10 11C10 11.5523 9.55228 12 9 12C8.44772 12 8 11.5523 8 11C8 10.4477 8.44772 10 9 10C9.55228 10 10 10.4477 10 11ZM12 11C12 12.6569 10.6569 14 9 14C7.34315 14 6 12.6569 6 11C6 9.34315 7.34315 8 9 8C10.6569 8 12 9.34315 12 11Z" fill="#5900E7"/></svg>',
				'keyword' => ['image', 'media']
			],
			'tp-infobox' => [
			...
			'tp-dynamic-category' => [
				...
				'videoUrl' => '',
				'tag' => 'pro',
				'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 36 36"><path fill="#1DD8A7" fill-rule="evenodd" d="M19 18a1 1 0 1 0-2 0c0 .217-.057.316-.132.393-.103.108-.327.254-.791.386-.948.27-2.463.364-4.677.364-1.941 0-3.302.528-4.19 1.329C6.336 21.26 6 22.246 6 23a1 1 0 1 0 2 0c0-.199.104-.641.55-1.043.432-.39 1.271-.814 2.85-.814 2.186 0 3.97-.084 5.223-.44.5-.142.979-.344 1.377-.648.398.304.877.506 1.377.648 1.253.356 3.037.44 5.223.44 1.578 0 2.418.424 2.85.814.446.402.55.844.55 1.043a1 1 0 1 0 2 0c0-.753-.336-1.74-1.21-2.528-.888-.8-2.248-1.33-4.19-1.33-2.214 0-3.73-.094-4.677-.363-.464-.132-.688-.278-.791-.386-.075-.077-.132-.176-.132-.393Z" clip-rule="evenodd"/><path fill="#5900E7" fill-rule="evenodd" d="M23 5H13v3.508l1.009-.948a1.5 1.5 0 0 1 1.927-.108l.97.728 2.012-2.012a1.5 1.5 0 0 1 2.182.064L23 8.37V5Zm-.338 6H13.27l1.803-1.696.98.736a1.5 1.5 0 0 0 1.96-.14l1.944-1.943L22.662 11ZM13 3a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2H13Zm-2 12a1 1 0 0 1 1-1h12a1 1 0 1 1 0 2H12a1 1 0 0 1-1-1ZM6 27h2v2H6v-2Zm-2 0a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v2a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-2Zm24 0h2v2h-2v-2Zm-2 0a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v2a2 2 0 0 1-2 2h-2a2 2 0 0 1-2-2v-2ZM4 33a1 1 0 0 1 1-1h4a1 1 0 1 1 0 2H5a1 1 0 0 1-1-1Zm23-1a1 1 0 1 0 0 2h4a1 1 0 1 0 0-2h-4Z" clip-rule="evenodd"/></svg>',
		...
		$this->block_extra = [
			'tp-advanced-border-radius' => [
				...
				'videoUrl' => '',
				'tag' => 'pro',
				'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 36 36"><path fill="#1DD8A7" fill-rule="evenodd" d="M11 6a1 1 0 0 0 0 2h14a1 1 0 1 0 0-2H11Zm19 5a1 1 0 0 0-2 0v14a1 1 0 1 0 2 0V11ZM10 29a1 1 0 0 1 1-1h14a1 1 0 0 1 0 2H11a1 1 0 0 1-1-1ZM8 11a1 1 0 0 0-2 0v14a1 1 0 1 0 2 0V11Z" clip-rule="evenodd"/><path fill="#5900E7" fill-rule="evenodd" d="M7 9a2 2 0 1 0 0-4 2 2 0 0 0 0 4Zm0 2a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm16.074 1.93c-1.339-1.292-3.222-2.006-5.136-1.924-1.754.076-3.361 1.03-4.698 2.388-1.203 1.223-1.845 2.794-2.043 4.544l-.002.01c-.128 1.132-.25 2.208-.17 3.2.08.974.347 1.764.922 2.37 1.268 1.338 3.511 1.62 5.866 1.427 1.898-.156 3.616-1.077 4.992-2.454 1.332-1.335 2.153-2.967 2.193-4.689.042-1.83-.614-3.608-1.924-4.872Zm-5.222-3.922c2.453-.105 4.872.804 6.61 2.482 1.754 1.693 2.59 4.028 2.535 6.359-.054 2.348-1.17 4.445-2.777 6.055-1.643 1.645-3.785 2.832-6.243 3.034-2.42.2-5.53.015-7.481-2.043-.996-1.051-1.363-2.34-1.464-3.586-.096-1.188.045-2.43.166-3.497l.011-.099c.239-2.103 1.03-4.12 2.605-5.721 1.54-1.566 3.597-2.879 6.038-2.984ZM9 29a2 2 0 1 1-4 0 2 2 0 0 1 4 0Zm2 0a4 4 0 1 1-8 0 4 4 0 0 1 8 0ZM29 9a2 2 0 1 0 0-4 2 2 0 0 0 0 4Zm0 2a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm2 18a2 2 0 1 1-4 0 2 2 0 0 1 4 0Zm2 0a4 4 0 1 1-8 0 4 4 0 0 1 8 0Z" clip-rule="evenodd"/></svg>',
			],
			'tp-content-hover-effect' => [
				...
				'videoUrl' => '',
				'tag' => 'pro',
				'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 36 36"><path fill="#5900E7" fill-rule="evenodd" d="M22.704 5.457A6 6 0 1 1 25 17v2a6 6 0 1 1-6 6h-2a6 6 0 1 1-6-6v-2a6 6 0 1 1 6-6h2a6 6 0 0 1 3.704-5.543ZM18 28.873a7.993 7.993 0 0 1-3.939 3.518 7.999 7.999 0 0 1-6.934-14.39A7.998 7.998 0 0 1 5.343 5.343 8 8 0 0 1 18 7.126 7.999 7.999 0 1 1 28.873 18 8 8 0 1 1 18 28.873Z" clip-rule="evenodd"/><path fill="#1DD8A7" fill-rule="evenodd" d="M18 21a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm0 2a5 5 0 1 0 0-10 5 5 0 0 0 0 10Z" clip-rule="evenodd"/></svg>',
			],
			'tp-continuous-animation' => [
				'label' => esc_html__('Continuous Animation', 'tpgb'),
				'demoUrl' => 'https://theplusblocks.com/plus-extras/global-tooltip/',
				'videoUrl' => '',
				'tag' => 'pro',
				'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 36 36"><path fill="#5900E7" fill-rule="evenodd" d="M22.704 5.457A6 6 0 1 1 25 17v2a6 6 0 1 1-6 6h-2a6 6 0 1 1-6-6v-2a6 6 0 1 1 6-6h2a6 6 0 0 1 3.704-5.543ZM18 28.873a7.993 7.993 0 0 1-3.939 3.518 7.999 7.999 0 0 1-6.934-14.39A7.998 7.998 0 0 1 5.343 5.343 8 8 0 0 1 18 7.126 7.999 7.999 0 1 1 28.873 18 8 8 0 1 1 18 28.873Z" clip-rule="evenodd"/><path fill="#1DD8A7" fill-rule="evenodd" d="M18 21a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm0 2a5 5 0 1 0 0-10 5 5 0 0 0 0 10Z" clip-rule="evenodd"/></svg>',
			],
			'tp-display-rules' => [
				...
				'demoUrl' => 'https://theplusblocks.com/plus-extras/display-rules/',
				'docUrl' => 'https://theplusblocks.com/help/display-rules/',
				'videoUrl' => '',
				'tag' => 'free',
				'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 36 36"><path fill="#1DD8A7" fill-rule="evenodd" d="M17.5 14c-2.49 0-4.5 2.01-4.5 4.5s2.01 4.5 4.5 4.5 4.5-2.01 4.5-4.5-2.01-4.5-4.5-4.5ZM11 18.5c0-3.594 2.906-6.5 6.5-6.5s6.5 2.906 6.5 6.5-2.906 6.5-6.5 6.5a6.494 6.494 0 0 1-6.5-6.5Z" clip-rule="evenodd"/><path fill="#5900E7" fill-rule="evenodd" d="M3.186 13.395C6.826 7.532 12.153 4 18 4c5.847 0 11.175 3.532 14.815 9.396C33.627 14.7 34 16.38 34 18c0 1.62-.373 3.3-1.185 4.605C29.175 28.468 23.847 32 18 32c-5.847 0-11.175-3.532-14.815-9.396C2.373 21.3 2 19.62 2 18c0-1.62.373-3.3 1.186-4.605ZM18 6c-4.975 0-9.734 3.003-13.116 8.45v.002C4.317 15.362 4 16.65 4 18s.317 2.638.884 3.548v.001C8.266 26.997 13.024 30 18 30c4.975 0 9.734-3.003 13.116-8.45v-.002c.567-.91.884-2.198.884-3.548s-.317-2.638-.884-3.548v-.001C27.734 9.003 22.976 6 18 6Z" clip-rule="evenodd"/></svg>',
			],
			'tp-equal-height' => [
				'label' => esc_html__('Equal Column Height', 'tpgb'),
				'demoUrl' => 'https://theplusblocks.com/plus-extras/equal-height/',
				'docUrl' => 'https://theplusblocks.com/help/equal-column-height/',
				'videoUrl' => '',
				'tag' => 'pro',
				'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 36 36"><path fill="#5900E7" fill-rule="evenodd" d="M14.154 4H5.846v28h8.308V4ZM5.846 2C4.826 2 4 2.895 4 4v28c0 1.105.827 2 1.846 2h8.308c1.02 0 1.846-.895 1.846-2V4c0-1.105-.827-2-1.846-2H5.846Z" clip-rule="evenodd"/><path fill="#1DD8A7" fill-rule="evenodd" d="M30 4h-9v28h9V4Zm-9-2a2 2 0 0 0-2 2v28a2 2 0 0 0 2 2h9a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2h-9Z" clip-rule="evenodd"/></svg>',
			],
			'tp-global-tooltip' => [
				...
				'videoUrl' => '',
				'tag' => 'pro',
				'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 36 36"><path fill="#1DD8A7" fill-rule="evenodd" d="M18 30a2 2 0 1 0 0-4 2 2 0 0 0 0 4Zm0 2a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z" clip-rule="evenodd"/><path fill="#5900E7" fill-rule="evenodd" d="M4 5.96h28v11.764H20.309c-.947 0-1.813.525-2.236 1.355l-.073.143-.073-.143a2.506 2.506 0 0 0-2.236-1.355H4V5.961Zm16.309 13.725a.5.5 0 0 0-.447.27l-1.415 2.774a.505.505 0 0 1-.894 0l-1.415-2.773a.501.501 0 0 0-.447-.271H4c-1.105 0-2-.878-2-1.96V5.96C2 4.878 2.895 4 4 4h28c1.105 0 2 .878 2 1.96v11.764c0 1.083-.895 1.96-2 1.96H20.309Zm-10.31-5.882c1.105 0 2-.878 2-1.96 0-1.084-.895-1.961-2-1.961-1.104 0-2 .877-2 1.96s.896 1.96 2 1.96Zm10-1.96c0 1.082-.895 1.96-2 1.96-1.104 0-2-.878-2-1.96 0-1.084.896-1.961 2-1.961 1.105 0 2 .877 2 1.96Zm6 1.96c1.105 0 2-.878 2-1.96 0-1.084-.895-1.961-2-1.961-1.104 0-2 .877-2 1.96s.896 1.96 2 1.96Z" clip-rule="evenodd"/></svg>',
			],
			'tp-magic-scroll' => [
				'label' => esc_html__('Magic Scroll', 'tpgb'),
				'demoUrl' => 'https://theplusblocks.com/plus-extras/magic-scroll/',
				'videoUrl' => '',
				'tag' => 'pro',
				'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 36 36"><path fill="#5900E7" fill-rule="evenodd" d="M9 11a8 8 0 0 1 8-8h12.414l-.003.003A3.75 3.75 0 0 1 33 6.75V10h-7v18.5a4.5 4.5 0 0 1-4.5 4.5H7a4 4 0 0 1-4-4v-3h6V11Zm2 15h7c0 1.777.5 3.508 1.429 5h2.07A2.5 2.5 0 0 0 24 28.5V8.5h.007A7.828 7.828 0 0 1 25 5h-8a6 6 0 0 0-6 6v15Zm6.148 5a11.46 11.46 0 0 1-.972-3H5v1a2 2 0 0 0 2 2h10.148ZM26.06 8a5.83 5.83 0 0 1 1.375-3h1.816c.966 0 1.75.784 1.75 1.75V8h-4.94Z" clip-rule="evenodd"/><path fill="#1DD8A7" fill-rule="evenodd" d="m14.5 10 1.04 2.46L18 13.5l-2.46 1.04L14.5 17l-1.04-2.46L11 13.5l2.46-1.04L14.5 10Zm-3.015 8.515L10 15l-1.485 3.515L5 20l3.515 1.485L10 25l1.485-3.515L15 20l-3.515-1.485Z" clip-rule="evenodd"/></svg>',
			],
			'tp-mouse-parallax' => [
				...
				'videoUrl' => '',
				'tag' => 'pro',
				'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 36 36"><path fill="#1DD8A7" fill-rule="evenodd" d="M16.5 9A1.5 1.5 0 0 1 15 7.5V5h-2v10h10V5h-2v2.5A1.5 1.5 0 0 1 19.5 9h-3Zm-4-6A1.5 1.5 0 0 0 11 4.5v11a1.5 1.5 0 0 0 1.5 1.5h11a1.5 1.5 0 0 0 1.5-1.5v-11A1.5 1.5 0 0 0 23.5 3h-3A1.5 1.5 0 0 0 19 4.5V7h-2V4.5A1.5 1.5 0 0 0 15.5 3h-3Z" clip-rule="evenodd" opacity=".7"/><path fill="#5900E7" fill-rule="evenodd" d="M7 23.5A1.5 1.5 0 0 0 8.5 25h3a1.5 1.5 0 0 0 1.5-1.5V21h2v10H5V21h2v2.5Zm-4-3A1.5 1.5 0 0 1 4.5 19h3A1.5 1.5 0 0 1 9 20.5V23h2v-2.5a1.5 1.5 0 0 1 1.5-1.5h3a1.5 1.5 0 0 1 1.5 1.5v11a1.5 1.5 0 0 1-1.5 1.5h-11A1.5 1.5 0 0 1 3 31.5v-11Zm20 3a1.5 1.5 0 0 0 1.5 1.5h3a1.5 1.5 0 0 0 1.5-1.5V21h2v10H21V21h2v2.5Zm-4-3a1.5 1.5 0 0 1 1.5-1.5h3a1.5 1.5 0 0 1 1.5 1.5V23h2v-2.5a1.5 1.5 0 0 1 1.5-1.5h3a1.5 1.5 0 0 1 1.5 1.5v11a1.5 1.5 0 0 1-1.5 1.5h-11a1.5 1.5 0 0 1-1.5-1.5v-11Z" clip-rule="evenodd"/></svg>',
			],
			'tp-scoll-animation' => [
				'label' => esc_html__('On Scroll Animation', 'tpgb'),
				'demoUrl' => 'https://theplusblocks.com/plus-extras/on-scroll-animations/',
				'videoUrl' => '',
				'tag' => 'freemium',
				'icon' => '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 36 36"><path fill="#5900E7" fill-rule="evenodd" d="M27 6H5a1 1 0 0 0-1 1v18a1 1 0 0 0 1 1h22a1 1 0 0 0 1-1V7a1 1 0 0 0-1-1ZM5 4a3 3 0 0 0-3 3v18a3 3 0 0 0 3 3h22a3 3 0 0 0 3-3V7a3 3 0 0 0-3-3H5Z" clip-rule="evenodd"/><path fill="#1DD8A7" d="M17.752 15.168a1 1 0 0 1 0 1.664l-3.197 2.131A1 1 0 0 1 13 18.131v-4.263a1 1 0 0 1 1.555-.832l3.197 2.132Z"/><path fill="#5900E7" fill-rule="evenodd" d="M33 8a1 1 0 0 1 1 1v19a4 4 0 0 1-4 4H7a1 1 0 1 1 0-2h23a2 2 0 0 0 2-2V9a1 1 0 0 1 1-1Z" clip-rule="evenodd"/></svg>',
			],
			'tp-3d-tilt' => [
	...
	public function tpgb_key_search( $lists = [], $filter =''){
		$filter_block =[];
		if(!empty($lists)){
			foreach($lists as $key => $block){
				...
				if(!empty($block['keyword'])){
					foreach($block['keyword'] as $keyword){
						...
						if(strpos($key_word, sanitize_text_field($filter)) !== false){
							$filter_block[$key]['filter'] = 'yes';
						}
					}
				}
				if(strpos($label, sanitize_text_field($filter)) !== false){
					$filter_block[$key]['filter'] = 'yes';
				}
			}
		}
	...
	public function tpgb_block_search(){
		...
		if(isset($_POST['filter']) && !empty($_POST['filter'])){
			$this->block_listout();
			$filter_block =[];
			$filter_block = $this->tpgb_key_search($this->block_lists, $_POST['filter'] );
			$this->block_lists = $filter_block;
			$filter_extra =[];
			$filter_extra = $this->tpgb_key_search($this->block_extra, $_POST['filter'] );
			$this->block_extra = $filter_extra;
		}else{
			$this->block_listout();
		}
		
		$output = $this->tpgb_block_list_rendered();
		$output .= $this->tpgb_block_extra_option();
		echo $output;
		exit();
	}
	...
	private function tpgb_block_extra_option(){
		$block_extra = $this->block_extra;
		$output ='';
		$get_blocks_save = get_option( 'tpgb_normal_blocks_opts' );
		$save_block ='';
		if( isset($get_blocks_save['tp_extra_option']) && !empty($get_blocks_save['tp_extra_option'])){
			$save_block = $get_blocks_save['tp_extra_option'];
		}
		
		if(!empty($block_extra)){
			$output .='<div class="tpgb-extras-feature-list">';										
				$output .='<h3 class="tpgb-extras-feature-list-title">'.esc_html__('Plus Extras' , 'tpgb').'</h3>';
				$output .='<p class="tpgb-extras-feature-list-desc">'.esc_html__('List of features added in The Plus Addon For Block Editor. You can enable/disable them from below.' , 'tpgb').'</p>';
			$output .= '</div>';
			foreach ($block_extra as $key => $blockext){
				$filter_class = '';
				if(!empty($blockext['filter'])){
					$filter_class = 'filter-block-'.esc_attr($blockext['filter']);
				}
				$output .='<div class="tpgb-panel-col tpgb-panel-col-25 block-'.esc_attr($blockext['tag']).' '.esc_attr($filter_class).'">';
					$output .='<div class="plus-block-list-wrap" data-list-block="'.esc_attr($key).'">';
						$output .='<div class="block-pin-free-pro">'.esc_html($blockext['tag']).'</div>';
						$output .='<div class="plus-block-list-inner">';
							$output .= (!empty($blockext['icon'])) ? '<span class="block-icon">'.$blockext['icon'].'</span>' : '';
							$output .='<span>'.esc_html($blockext['label']).'</span>';
							$output .='<span class="block-group-info">';
								$output .='<span class="block-hover-info">';
								$output .='<svg xmlns="http://www.w3.org/2000/svg" width="3.75" height="10.8" viewBox="0 0 4.449 11.917"><defs/><path d="M3.604 3.602H.061v1.5h.962v5.364H0v1.435h1.021v.01h2.583v-.01h.845v-1.429h-.845z" data-name="Path 348"/><path d="M2.294 2.582A1.294 1.294 0 102.329 0a1.293 1.293 0 10-.035 2.582z" data-name="Path 349"/></svg>';
							$output .='</span>';
							$output .='<a href="'.esc_url($blockext['demoUrl']).'" target="_blank" class="block-hover-details block-info-demo">';
								$output .='<svg xmlns="http://www.w3.org/2000/svg" width="10" height="9.009"><defs/><path d="M9.755.257a.787.787 0 00-.589-.255H.833a.788.788 0 00-.589.255A.851.851 0 000 .866v5.891a.851.851 0 00.245.612.788.788 0 00.589.254h2.833a1.171 1.171 0 01-.083.422 3.905 3.905 0 01-.167.379.614.614 0 00-.083.238.339.339 0 00.1.244.314.314 0 00.234.1h2.665a.314.314 0 00.234-.1.339.339 0 00.1-.244.635.635 0 00-.083-.235 4.052 4.052 0 01-.167-.384 1.18 1.18 0 01-.083-.42h2.833a.787.787 0 00.588-.254.851.851 0 00.245-.612V.867a.851.851 0 00-.245-.61zm-.422 5.116a.17.17 0 01-.049.122.158.158 0 01-.117.051H.833a.157.157 0 01-.117-.051.17.17 0 01-.049-.122v-4.5A.17.17 0 01.716.751.158.158 0 01.833.7h8.334a.157.157 0 01.117.051.17.17 0 01.049.122v4.5z"/></svg>';
							$output .='</a>';
							if( isset($blockext['docUrl']) && !empty($blockext['docUrl']) ){
								$output .='<a href="'.esc_url($blockext['docUrl']).'" target="_blank" class="block-hover-details block-info-doc">';
									$output .='<svg xmlns="http://www.w3.org/2000/svg" width="8.053" height="10.166" viewBox="0 0 8.053 10.166">
									<g transform="translate(-41.796)">
										<g transform="translate(42.06 1.188)">
										  <path d="M226.884,303.02l-2.231,2.218v-1.69a.528.528,0,0,1,.528-.528Z" transform="translate(-220.296 -296.551)"/>
							...
							if( isset($blockext['videoUrl']) && !empty($blockext['videoUrl']) ){
								$output .='<a href="'.esc_url($blockext['videoUrl']).'" target="_blank" class="block-hover-details block-info-video">';
									$output .='<svg xmlns="http://www.w3.org/2000/svg" width="7.801" height="10.037"><defs/><path d="M.4 10.036a.4.4 0 01-.4-.4V.399A.4.4 0 01.62.065l7 4.618a.4.4 0 01.181.334.4.4 0 01-.181.334l-7 4.619a.4.4 0 01-.22.066zm.4-8.894v7.749l5.874-3.876z"/></svg>';
								$output .='</a>';
							}
							$output .='</span>';
						$output .='</div>';
						$pro_disable = '';
						if(!defined('TPGBP_VERSION') && $blockext['tag']=='pro'){
							$pro_disable = 'disabled="disabled"';
						}
						$checked = '';
						if(!empty($save_block) && in_array($key, $save_block)){
						...
						$output .='<div class="block-check-wrap"><input type="checkbox" class="block-list-checkbox" name="tp_extra_option[]" id="'.esc_attr($key).'" value="'.esc_attr($key).'" '.$checked.' '.$pro_disable.'> <label for="'.esc_attr($key).'"></label></div>';
					$output .='</div>';
				$output .='</div>';
			}
		}
	...
	/**
	 * Theplus Extra Options HTML
	 * 
	 */
	public function tpgb_connection_data_html($formsdata){
		$output = '';
		$get_extra_options = get_option('tpgb_connection_data');
		$output .='<div class="tpgb-settings-wrap tpgb-connection-all-fields">';
			foreach ($formsdata as $fData):
				$conditionData = $hideItem = '';
				if(isset($fData['attributes']) && !empty($fData['attributes'])){
					if(isset($fData['attributes']['data-conditional-id']) && !empty($fData['attributes']['data-conditional-id'])){
						$hideItem = 'style="display: none"';
						$getCndId = $fData['attributes']['data-conditional-id'];
						$conditionData = 'data-condition-id="'.esc_attr($getCndId).'"';
						if(empty($get_extra_options)){
							$hideItem = '';
						}else if(!empty($get_extra_options) && isset($get_extra_options[$getCndId]) && $get_extra_options[$getCndId] == 'enable'){
							$hideItem = '';
						}
					}
				}

				$output .='<div class="tpgb-extra-opt-wrap '.esc_attr($fData['id']).'-item" '.$conditionData.' '.$hideItem.'>';
					$output .='<div class="tpgb-settings-head-title"><label>'.wp_kses_post($fData['name']).'</label></div>';
					$output .='<div class="tpgb-extra-main-data">';
						if($fData['type']=='select'){
							$output .='<select class="extra-data-select" name="'.esc_attr($fData['id']).'" id="'.esc_attr($fData['id']).'">';
								foreach($fData['options'] as $option => $val){
									$selected = '';
									if(!empty($get_extra_options) && isset($get_extra_options[$fData['id']]) && $get_extra_options[$fData['id']] == $option){
										$selected = 'selected';
									}
									$output .='<option value="'.esc_attr($option).'" '.$selected.'>'.wp_kses_post($val).'</option>';
								}
							$output .='</select>';
						}else if($fData['type']=='text'){
							$defVal = (isset($fData['default']) && !empty($fData['default'])) ? $fData['default'] : '';
							$gtValue = (!empty($get_extra_options) && isset($get_extra_options[$fData['id']]) && !empty($get_extra_options[$fData['id']])) ? $get_extra_options[$fData['id']] : $defVal;
							$output .='<input name="'.esc_attr($fData['id']).'" id="'.esc_attr($fData['id']).'" value="'.esc_attr($gtValue).'"/>';

							if( isset( $fData['after_field'] ) && !empty( $fData['after_field'] ) ){
								$output .= $fData['after_field'];
							}
						}
						$output .='<p class="extra-item-description">'.wp_kses_post($fData['desc']).'</p>';
					$output .='</div>';
				$output .='</div>';
			endforeach;
		$output .='</div>';
	...
	/**
	 * Theplus Custom CSS/JS HTML
	 * 
	 */
	public function tpgb_custom_editor_html($formsdata){
		$output = '';
		$get_custom_options = get_option('tpgb_custom_css_js');

		$output .='<div class="tpgb-settings-wrap">';
			foreach ($formsdata as $fData):

				$gtValue = (!empty($get_custom_options) && isset($get_custom_options[$fData['id']]) && !empty($get_custom_options[$fData['id']])) ? $get_custom_options[$fData['id']] : '';

				if($fData['id'] != 'tpgb_custom_hidden'){
					$output .='<div class="'.esc_attr($fData['id']).'_item">';
						$output .='<div class="tpgb-settings-head-title"><label>'.esc_attr($fData['name']).'</label></div>';
						$output .='<div class="tpgb-custom-main-data">';
							$output .='<pre>';
								$output .='<textarea name="'.esc_attr($fData['id']).'" id="'.esc_attr($fData['id']).'">'.$gtValue.'</textarea>';
							$output .='</pre>';
							$output .='<p class="tpgb-custom-desc">'.wp_kses_post($fData['desc']).'</p>';
						$output .='</div>';
					$output .='</div>';
				}else{
					$output .='<input type="hidden" name="'.esc_attr($fData['id']).'" id="'.esc_attr($fData['id']).'" value="hidden">';
				}
			endforeach;
		$output .='</div>';
		return $output;
	...
	/**
	 * Theplus White Label HTML
	 * 
	 */
	public function tpgb_white_label_html($formsdata){
		$output = '';
		$get_wl_options = get_option('tpgb_white_label');
		$output .='<div class="tpgb-settings-wrap">';
			foreach ($formsdata as $fData):
				if($fData['id'] != 'tpgb_white_label_hidden'){
					$output .='<div class="tpgb_white_label_item item-'.esc_attr($fData['id']).'">';
						$output .='<div class="tpgb-settings-head-title"><label>'.esc_attr($fData['name']).'</label></div>';
						$output .='<div class="tpgb-whitel-main-data">';
						if($fData['type']=='text' || $fData['type']=='text_url'){
							$text_val = (!empty($get_wl_options)) ? (!empty($get_wl_options[$fData['id']]) ? $get_wl_options[$fData['id']] : '') : '';
							$output .='<input type="text" name="'.$fData['id'].'" id="'.esc_attr($fData['id']).'" value="'.$text_val.'" placeholder="'.esc_attr($fData['attributes']['placeholder']).'" autocomplete="'.esc_attr($fData['attributes']['autocomplete']).'">';
						}else if($fData['type']=='textarea_small'){
							$textarea_val = (!empty($get_wl_options)) ? (( isset($get_wl_options[$fData['id']]) && !empty($get_wl_options[$fData['id']])) ? $get_wl_options[$fData['id']] : '') : '';
							$output .='<textarea type="text" name="'.esc_attr($fData['id']).'" id="'.esc_attr($fData['id']).'" placeholder="'.esc_attr($fData['attributes']['placeholder']).'" autocomplete="'.esc_attr($fData['attributes']['autocomplete']).'">'.wp_kses_post($textarea_val).'</textarea>';
						}else if($fData['type']=='file'){
							$logoid = (!empty($get_wl_options)) ? (!empty($get_wl_options['tpgb_plus_logo_id']) ? 'value="'.esc_attr($get_wl_options['tpgb_plus_logo_id']).'"' : '') : '';
							$logourl = (!empty($get_wl_options)) ? (!empty($get_wl_options['tpgb_plus_logo']) ? 'value="'.esc_attr($get_wl_options['tpgb_plus_logo']).'"' : '') : '';
							$logosrc = (!empty($get_wl_options)) ? (!empty($get_wl_options['tpgb_plus_logo']) ? 'src="'.esc_url($get_wl_options['tpgb_plus_logo']).'"' : '') : '';
							$output .='<input class="tpgb-logo-btn" type="button" value="'.esc_attr__('Add or Upload File','tpgb').'">';
							$output .='<input type="hidden" name="tpgb_plus_logo" id="tpgb_plus_logo" '.$logourl.'>';
							$output .='<input type="hidden" name="tpgb_plus_logo_id" id="tpgb_plus_logo_id" '.$logoid.'>';

							$hideclose = '';
							if(!empty($logoid) && !empty($logourl)){
								$hideclose = 'style="display: flex"';
							}

							$output .='<div class="tpgb_plus_logo-status" '.$hideclose.'><img class="tpgb-selected-logo-image" '.$logosrc.'/><a class="tpgb-rempve-wl-logo"></a></div>';
						}else if($fData['type']=='checkbox'){
							$output .='<input type="checkbox" name="'.esc_attr($fData['id']).'" id="'.esc_attr($fData['id']).'" value="on">';
							$output .='<label for="'.esc_attr($fData['id']).'"><span class="hide-wl-desc">'.esc_attr($fData['desc']).'</span></label>';
						}
						$output .='</div>';
					$output .='</div>';
				}else{
	...
	/**
	 * Theplus Performance HTML
	 * 
	 */
	public function tpgb_performance_html(){
		$output = '';
		$get_perf = get_option('tpgb_performance_cache');
		$getSel = (!empty($get_perf)) ? $get_perf : 'combine';
		$combineSel = ($getSel == 'combine') ? 'selected="selected"' : '';
		$seprateSel = ($getSel == 'separate') ? 'selected="selected"' : '';
		$hideSep = $hideCom = '';
		if($getSel == 'combine'){
			$hideSep = 'style="display: none"';
		}else{
			$hideCom = 'style="display: none"';
		}

		$get_delay = get_option('tpgb_delay_css_js');
		$delayCheck = (!empty($get_delay) && $get_delay == 'true') ? 'checked' : '';
		$get_defer = get_option('tpgb_defer_css_js');
		$deferCheck = (!empty($get_defer) && $get_defer == 'true') ? 'checked' : '';

		$output .='<div class="tpgb-settings-wrap tpgb-performance-wrap">';
			$output .='<div class="tpgb_perf_row tpgb-remove-plus-cache">';
				$output .='<div class="tpgb-settings-head-title"><label for="plus_smart_performance">'.esc_html__("CSS & JS Delivery System", "tpgb").'</label></div>';
					$output .='<div class="tpgb-select-caching-type">';
						$output .='<select id="tpgb-cache-opt-performance">';
							$output .='<option value="combine" '.$combineSel.'>'.esc_html__("Smart Optimized Mode", "tpgb").'</option>';
							$output .='<option value="separate" '.$seprateSel.'>'.esc_html__("On Demand Assets", "tpgb").'</option>';
						$output .='</select>';
						$output .='<div class="tpgb-perf-save-msg"></div>';
					$output .='</div>';
					$output .='<div class="tpgb_pref_data performance-combine" '.$hideCom.'>';
						$output .='<p class="perf-description">'.esc_html__('On First Load, CSS delivery start from header in minified-combined format and One combined-minified JS in the footer. On Second load, All CSS will be combined-minified in just one single CSS file and loaded on header and JS will be delivered from footer same as First load. The optimized asset is stored at "/wp-content/uploads/theplus_gutenberg"', 'tpgb').'</p>';
						$output .='<a href="#" id="tpgb-remove-smart-cache" class="tpgb-purge-cache-btn">'.esc_html__('Purge All Assets', 'tpgb').'</a>';
						$output .='<div class="smart-performace-desc-btn">'.esc_html__('*Above button will remove all Combined-minified files from "/wp-content/uploads/theplus_gutenberg" and It will regenerate on your page visit.', 'tpgb').'</div>';
					$output .='</div>';
					$output .='<div class="tpgb_pref_data performance-separate" '.$hideSep.'>';
					$output .='<p class="perf-description">'.esc_html__('Based on blocks used on web page, Individual CSS for each block will be loaded from header. All JS will be loaded in footer individually based on blocks used on that page.', 'tpgb').'</p>';
				$output .='</div>';

				$output .='<div class="tpgb-delay-defer-wrap">';
					$output .='<div class="tpgb-smart-loader-delay tpgb-remove-dynamic-style">';
						$output .='<label>'.esc_html__('Delay Extra JS ', 'tpgb').'<span class="tpgb-tooltip-dynamic">!</span></label>';
						$output .='<div class="block-check-wrap">';
							$output .='<input type="checkbox" class="block-list-checkbox" name="tpgb-smart-loader" id="tp-delay-js-opt" '.$delayCheck.'/>';
							$output .='<label for="tp-delay-js-opt"></label>';
						$output .='</div>';
					$output .='</div>';
					$output .='<div class="tpgb-smart-loader-delay tpgb-remove-dynamic-style tpgb-load-defer">';
						$output .='<label>'.esc_html__('Defer CSS & JS ', 'tpgb').'<span class="tpgb-tooltip-dynamic">!</span></label>';
						$output .='<div class="block-check-wrap">';
							$output .='<input type="checkbox" class="block-list-checkbox" name="tpgb-smart-loader" id="tp-defer-js-opt" '.$deferCheck.'/>';
							$output .='<label for="tp-defer-js-opt"></label>';
						$output .='</div>';
					$output .='</div>';
				$output .='</div>';
			...
			$output .='<div class="tpgb_perf_row tpgb-remove-dynamic-style">';
				$output .='<div class="tpgb-settings-head-title">';
					$output .='<label for="plus_smart_performance">'.esc_html__('Regenerate Assets ', 'tpgb').'<span class="tpgb-tooltip-dynamic">!</span></label>';
					$output .='<p class="perf-description">'.esc_html__('Note : If you find any discrepancy in frontend and backend design, then we would suggest you to click on above button to regenerate the assets dynamically again based on design changes.', 'tpgb').'</p>';
				$output .='</div>';
				$output .='<div class="tpgb_pref_data">';
					$output .='<a href="#" id="tpgb-remove-dynamic-style" class="tpgb-smart-cache-btn">'.esc_html__('REGENERATE ALL ASSETS', 'tpgb').'</a>';
				$output .='</div>';
			$output .='</div>';
		$output .='</div>';
						...
						foreach ($tab_forms as $tab_form):
							if($tab_form['id']=='tpgb_normal_blocks_opts'){
								echo '<div class="tpgb-panel-plus-block-page">';
									...
									/*block listing*/
									echo '<form action="'.esc_url( admin_url('admin-post.php') ).'" method="post" id="tpgb_normal_blocks_opts" enctype="multipart/form-data" encoding="multipart/form-data">';
										...
										echo '<div class="tpgb-panel-row tpgb-mt-50 plus-block-list '.esc_attr($is_pro).'">';
											...
											echo $this->tpgb_block_extra_option();
										echo '</div>';
							...
							if(defined('TPGBP_VERSION') && $tab_form['id']=='tpgb_white_label'){
								echo '<form action="'.esc_url( admin_url('admin-post.php') ).'" method="post" id="tpgb_white_label" enctype="multipart/form-data" encoding="multipart/form-data">';
									wp_nonce_field( 'nonce_tpgb_white_label_action', 'nonce_tpgb_white_label' );
									echo $this->tpgb_white_label_html($tab_form['fields']);
									echo '<input type="hidden" name="action" value="tpgb_white_label_save">';
									echo '<input type="submit" name="submit-key" value="Save" class="button-primary tpgb-submit-block">';
								echo '</form>';
							}else if($tab_form['id']=='tpgb_performance'){
								echo '<form action="'.esc_url( admin_url('admin-post.php') ).'" method="post" id="tpgb_performance" enctype="multipart/form-data" encoding="multipart/form-data">';
									wp_nonce_field( 'nonce_tpgb_performance_action', 'nonce_tpgb_performance' );
									echo $this->tpgb_performance_html($tab_form['fields']);
								echo '</form>';
							}else if($tab_form['id']=='tpgb_connection_data'){
								echo '<form action="'.esc_url( admin_url('admin-post.php') ).'" method="post" id="tpgb_connection_data" enctype="multipart/form-data" encoding="multipart/form-data">';
									wp_nonce_field( 'nonce_tpgb_connection_data_action', 'nonce_tpgb_connection_data' );
									echo $this->tpgb_connection_data_html($tab_form['fields']);
									echo '<input type="hidden" name="action" value="tpgb_connection_data_save">';
									echo '<input type="submit" name="submit-key" value="Save" class="button-primary tpgb-submit-block">';
								echo '</form>';

								do_action('tpgb_rollback_render_content');
							}else if($tab_form['id']=='tpgb_custom_css_js'){
								echo '<form action="'.esc_url( admin_url('admin-post.php') ).'" method="post" id="tpgb_custom_css_js" enctype="multipart/form-data" encoding="multipart/form-data">';
									wp_nonce_field( 'nonce_tpgb_custom_css_js_action', 'nonce_tpgb_custom_css_js' );
									echo $this->tpgb_custom_editor_html($tab_form['fields']);
									echo '<input type="hidden" name="action" value="tpgb_custom_css_js_save">';
									echo '<input type="submit" name="submit-key" value="Save" class="button-primary tpgb-submit-block">';
								echo '</form>';
    ...
    public function option_fields($verify_api='') {
		...
		$extra_options= array(
			...
			array(
				'name' => esc_html__('AJAX Templates','tpgb'),
				'desc' => sprintf( esc_html__('You can enable this option to add option of AJAX based loading to your reusable blocks. %s', 'tpgb'),'<a href="#" target="_blank" rel="noopener noreferrer" style="text-decoration: none;">'.esc_html__( 'Read more','tpgb' ).'</a>' ),
				'id'   => 'tpgb_template_load',
				'type' => 'select',	
				'default' => 'enable',
				'options' => array(
					'enable' => esc_html__('Enable', 'tpgb'),
			...
			array(
				'name' => esc_html__('Lazy Rendering ( SEO Friendly )','tpgb'),
				'desc' => sprintf( esc_html__('If enabled, You can start using class %1$s in places, where you want to lazy load content with SEO friendly way. %2$s', 'tpgb') , '<b> nxt-lazy-load </b>' ,'<a href="#" target="_blank" rel="noopener noreferrer" style="text-decoration: none;">'.esc_html__( 'How to Use?','tpgb' ).'</a>' ),
				'id'   => 'tpgb_lazy_render',
				'type' => 'select',	
				'default' => 'enable',
				'options' => array(
    ...
    public function __get($field)
    {
        ...
        /* translators: Invalid property: %s */
	...
	public function tpgb_boarding_store($CutonData , $load=""){
		...
		$tponbData = ( isset($_POST['boardingData']) && !empty($_POST['boardingData']) ) ? (array) json_decode( wp_unslash($_POST['boardingData'])) : $CutonData;
		$userData = [];
