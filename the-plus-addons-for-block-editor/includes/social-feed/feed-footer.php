    ...
    if($selectFeed == 'Facebook'){
        if(isset($Fblikes)){
            echo '<span class="tpgb-btn-like">';
                if(!empty($showFooterIn) && $style == "style-2" ){
                    echo esc_html__("Like ",'the-plus-addons-for-block-editor');
                }else{
                    echo '<img src="'.esc_url($likeImg).'" alt="'.esc_attr__('Like','the-plus-addons-for-block-editor').'"/>';
                    echo '<img src="'.esc_url($ReactionImg).'" alt="'.esc_attr__('Reaction','the-plus-addons-for-block-editor').'"/>';
                }
                echo esc_html(tpgb_number_short($Fblikes));
            echo '</span>';
        }
        if(isset($comment)){
            echo '<span class="tpgb-btn-comment">';
                if(!empty($showFooterIn) && $style == "style-2" ){
                    echo esc_html__('comment ','the-plus-addons-for-block-editor');
                }else{
        ...
        if(isset($share)){
            echo '<span class="tpgb-btn-share">';
                if(!empty($showFooterIn) && $style == "style-2" ){
                    echo esc_html__("share ",'the-plus-addons-for-block-editor');
                }else{
